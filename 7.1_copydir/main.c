...
// argv[1] -- родительский каталог, где надодится копируемая дериктория
// argv[2] -- копируемая дериктория
...
int main(int argc, char* argv[]) {
    ...
    int sys_dir_fd = openat(base_dir, "Other_things", O_RDONLY);
    if (sys_dir_fd < 0) {
        perror("Can't open current directory");
        return RESULT_OPEN_FAILED;
    }
    ...
    struct stat dir_sb;
    assert((fstatat(base_dir, "Other_things", &dir_sb, AT_SYMLINK_NOFOLLOW)) == 0);
    // --------------------------------------------------------------------------------------------
    
    // создаем в родительской дериктоии каталог-копию с правами исходника
    if (mkdirat(base_dir, "CopyDir", dir_sb.st_mode) < 0) {
    ...
    int sys_copy_dir = openat(base_dir, "CopyDir", O_RDONLY);
    if (sys_copy_dir == -1) {
